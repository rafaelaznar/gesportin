...
@Component({
	selector: 'app-categoria-plist-admin-unrouted',
	imports: [BotoneraRpp, Paginacion, RouterLink],
	templateUrl: './categoria-plist-admin-unrouted.html',
	styleUrl: './categoria-plist-admin-unrouted.css',
	standalone: true,
})
export class CategoriaPlistAdminUnrouted {
	// filtro opcional cuando el componente se usa desde el routed (p. ej. /categoria/temporada/:id)
	...
	// total (si hay filtro/búsqueda, mostramos los registros resultantes)
		...
		this.oCategoriaService
			...
			.subscribe({
				next: (data: IPage<ICategoria>) => {
					// normalizamos el comportamiento: el backend ya filtra, pero si no lo hiciera
