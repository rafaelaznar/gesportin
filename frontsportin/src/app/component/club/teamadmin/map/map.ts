...
@Component({
  standalone: true,
  selector: 'app-club-teamadmin-map',
  imports: [CommonModule],
  templateUrl: './map.html',
  styleUrls: ['./map.css'],
})
export class ClubTeamadminMap implements OnInit, AfterViewInit {
  ...
  get mapId(): string {
    return `clubMap-${this.club?.id ?? 'unknown'}`;
  }

  ngOnInit(): void {
    if (!this.club) {
      this.error.set('Club no disponible para el mapa.');
    }
  }

  ngAfterViewInit(): void {
    const lat = this.club?.latitud;
    const lng = this.club?.longitud;
